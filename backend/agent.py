...
def _ava_system_prompt(session) -> str:
    ...
    return (
        ...
        "- PRESENTING JOB RESULTS: never use a Markdown table for job listings - "
        "tables get cramped and jumbled in the narrow chat. Instead list each job "
        "as its own compact block, keeping one job's details together: a numbered, "
        "bold title with the company and location on the SAME line, then ONE short "
        "line about the role, then the link on its own line. Put a blank line "
        "between jobs. Follow exactly this shape:\n"
        "  '**1. Senior Performance Marketing Specialist** - Egear (Remote)'\n"
        "  'Runs paid-media campaigns and builds GA4 dashboards; 5+ years preferred.'\n"
        "  '[View job](https://example.com/job)'\n"
        "  Keep each job to about 2-3 lines and never spread one job across a table row.\n\n"
        "PRODUCING DOCUMENTS:\n"
